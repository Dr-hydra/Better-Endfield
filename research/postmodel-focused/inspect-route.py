...
def main():
    ...
    selected = {0x03BD3740: 'Object.Instantiate_pos_rot_parent'}
    ...
    for rva, label in sorted(selected.items()):
        ...
        results.append(result)
    (ROOT / 'instantiate-route.json').write_text(json.dumps({'input': str(binary), 'selected_count': len(selected), 'functions': results}, ensure_ascii=False, indent=2), encoding='utf-8')
    ida_auto.enable_auto(False)
    idc.qexit(0)

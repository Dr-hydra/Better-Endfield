...
local PANEL_ID = PanelId.ActivityCoinRanking
local PHASE_ID = PhaseId.ActivityCoinRanking
...
local LocateSelfAnim = "activitycoinrankinglistcell_in"

local scoreRankValueConverter = function(value)
    return string.format("%d", math.floor(value or 0))
end

ActivityCoinRankingCtrl = HL.Class('ActivityCoinRankingCtrl', uiCtrl.UICtrl)

ActivityCoinRankingCtrl.m_activityId = HL.Field(HL.String) << ""

ActivityCoinRankingCtrl.m_rankRelatedId = HL.Field(HL.String) << ""

ActivityCoinRankingCtrl.m_genRankListCellFunc = HL.Field(HL.Function)

ActivityCoinRankingCtrl.m_rawRankInfoDic = HL.Field(HL.Userdata)

ActivityCoinRankingCtrl.m_rankInfo = HL.Field(HL.Table)

ActivityCoinRankingCtrl.m_selfRankInfo = HL.Field(HL.Table)

ActivityCoinRankingCtrl.m_rankInfoReady = HL.Field(HL.Boolean) << false

ActivityCoinRankingCtrl.m_roleSimpleInfoReady = HL.Field(HL.Boolean) << false

ActivityCoinRankingCtrl.m_scrollToSelfTag = HL.Field(HL.Boolean) << false





ActivityCoinRankingCtrl.s_messages = HL.StaticField(HL.Table) << {
    [MessageConst.ON_ACTIVITY_REQ_RANK_LIST] = 'OnActivityReqRankList',
    [MessageConst.ON_FRIEND_INFO_SYNC] = "OnFriendInfoSync",
}


ActivityCoinRankingCtrl.OnCreate = HL.Override(HL.Any) << function(self, arg)
    self:_InitUI()
    self:_InitData(arg)
    self:_InitView()
end
...
ActivityCoinRankingCtrl.OnClose = HL.Override() << function(self)
    GameInstance.player.friendSystem:ClearSyncCallback()
end

ActivityCoinRankingCtrl.OnActivityReqRankList = HL.Method(HL.Table) << function(self, args)
...
ActivityCoinRankingCtrl.OnFriendInfoSync = HL.Method() << function(self)
    self.m_roleSimpleInfoReady = true

    self:_TryRefresh()
end

ActivityCoinRankingCtrl._InitUI = HL.Method() << function(self)
...
ActivityCoinRankingCtrl._InitData = HL.Method(HL.Table) << function(self, args)
    self.m_activityId = args.activityId
    self.m_rankRelatedId = args.rankRelatedId
end

ActivityCoinRankingCtrl._InitView = HL.Method() << function(self)
...
ActivityCoinRankingCtrl._RefreshData = HL.Method() << function(self)
    self.m_rankInfo = {}
    self.m_selfRankInfo = {}
...
ActivityCoinRankingCtrl._RefreshView = HL.Method() << function(self)
...
ActivityCoinRankingCtrl._TryRefresh = HL.Method() << function(self)
...
ActivityCoinRankingCtrl._OnClickBtnClose = HL.Method() << function(self)
    PhaseManager:PopPhase(PHASE_ID)
end

ActivityCoinRankingCtrl._OnUpdateCellRankingCell = HL.Method(GameObject, HL.Number) << function(self, go, csIndex)
    ...
    local playerHeadLight = cell.commonPlayerHead.view.playerHeadBtn.transform:Find("Light")
    if playerHeadLight then
        playerHeadLight.gameObject:SetActiveIfNecessary(isPlayer)
    end

    cell.timeTxt.text = scoreRankValueConverter(rankInfo.rankValue)
...
ActivityCoinRankingCtrl._GetRankingStateByRanking = HL.Method(HL.Number).Return(HL.String) << function(self, ranking)
...
ActivityCoinRankingCtrl._RefreshSelfInfo = HL.Method() << function(self)
    local selfInfo = GameInstance.player.friendSystem.SelfInfo
    ...
    selfNode.timeTxt.text = self.m_selfRankInfo.rankValue and
            scoreRankValueConverter(self.m_selfRankInfo.rankValue) or "-"
...
ActivityCoinRankingCtrl._OnClickSelfJumpBtn = HL.Method() << function(self)
...
ActivityCoinRankingCtrl._OnScrollEndRankingScrollList = HL.Method() << function(self)
...
ActivityCoinRankingCtrl._PlayLocateSelfAnim = HL.Method() << function(self)
    local luaIndex = self.m_selfRankInfo.rankLuaIndex
    local rankCell = self.m_genRankListCellFunc(luaIndex)
    if not rankCell then
        logger.error("ActivityCoinRankingCtrl._PlayLocateSelfAnim cant get rankCell, rankIndex:", luaIndex)
        return
    end
    rankCell.animationWrapper:Play(LocateSelfAnim)
end

ActivityCoinRankingCtrl._EventLogRankingView = HL.Method() << function(self)
    local eventRankInfo = {}
...
HL.Commit(ActivityCoinRankingCtrl)

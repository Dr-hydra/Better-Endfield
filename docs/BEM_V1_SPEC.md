...
2026-09-18；对应本分支读写器。扩展名 `.bem`。这是正式协议；BEMPC24/25 仅为内部研究格式，运行时与管理 UI 不接受。
当前实现平台为 Windows x64。Android 未实现，不把 PC 验证结果视作 Android 兼容承诺。
...
当前读取器仅接受 1.0；后续 minor 扩展必须保持已有字段语义，并升级读取器明确放行。

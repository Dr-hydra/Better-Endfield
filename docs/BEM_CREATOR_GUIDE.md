...
自 3.3.0 起 Windows 与 Android 双端可用，使用同一个标准包：Android 直接编译 `native/modules/custom_model` 的桌面源码，解析与校验路径两端一致，作者无需为手机端另出一份包。包头 `target.platform` 固定为 `windows-x64`，这是格式常量而非平台限制。作者需要知道的唯一双端差异是纹理：手机 GPU 纹理格式与桌面不同，Android 端在导入后可按包执行「转换手机纹理」，该步骤依赖包内已验证的法线编码信息，缺少时可以导入但无法转换——因此打包时应确保法线贴图的编码声明完整。两端切换包或外观后都需要重启游戏。完整字段见同目录 `BEM_V1_SPEC.md`。
...
每部件最多 256 个局部骨骼、256 个 draw；每个选定外观最多 32 个纹理绑定、512 MiB 上传数据预算。
源 16 位骨骼索引会重映射，输出是 UINT8 局部索引。超限会报告失败；没有自动分片、低 LOD 生成。
不执行源热键脚本、任意 Shader、运行时形态切换或形态键。多个外观由管理 UI 选择，下次启动生效。
...
导入会验证全部外观。导入/更新/删除时关闭游戏，避免延迟加载读取正在变动的文件；开关与外观选择可先保存，下一次启动生效。
更新采用相同 `package_id`；保留本机启用状态及仍存在的外观 ID，移除的外观会提示并回退默认项。

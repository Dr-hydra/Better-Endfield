...
def main():
    ...
    for rva, label in sorted(selected.items()):
        ...
        results.append(result)
    (ROOT / 'delivery-confirm.json').write_text(json.dumps({'input': str(binary), 'selected_count': len(selected), 'functions': results}, ensure_ascii=False, indent=2), encoding='utf-8')
    ida_auto.enable_auto(False)
    idc.qexit(0)

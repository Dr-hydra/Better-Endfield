...
- [x] 阶段 2：VMD 解析器独立成共享代码
- [ ] 阶段 3：VMD 身体动作播放（大头，需多轮实机测试；已完成可移植核心与首轮运行时接入）
- [ ] 阶段 4：面部表情、眼睛、裙子布料
...
- 已完成为 `native/shared/include/BetterEndfield/vmd_parser.h`，运行时只负责文件读取和相机数据适配。
- 解析器独立于 Windows 和 Unity，支持骨骼、表情、相机、灯光、自阴影及 IK 开关帧；保留 VMD 的固定宽度字符串字节，不在解析层强行转换 Shift-JIS。
- 对每个区段和 IK 子项设置数量上限，检查所有记录边界、浮点数和相机 FOV；同一帧的重复记录按 VMD 常见行为保留最后一条。
- `native/tests/vmd_parser_tests.cpp` 覆盖最小文件、可选区段、重复帧、无效 FOV、截断输入和数量限制。

阶段 2 补充：骨骼与表情记录现在保留固定宽度名称，并按“名称＋帧号”去重；新增 `vmd_motion.h` 提供无 Unity 依赖的时间线分组、Bezier/Slerp 采样和 morph/IK 采样。
...
当前实现：新增 `vmd_body_runtime.inc`。播放由配置中的 `vmd_body_file` 和 `vmd_body_play_hotkey` 触发，运行时扫描当前角色层级，按原始 VMD 名称或 CP932 解码后的名称匹配 Transform，保存 localPosition/localRotation，播放结束或退出时恢复。所有写入均通过动态 Transform 契约；没有使用 EIEM 的 IL2CPP 字段偏移。当前只覆盖通用 FK 骨骼，手指扭转、FinalIK 腿部和地形吸附仍需实机观测后分步加入。
...
当前实现：身体播放文件中的 morph 记录会在可用时通过 `Mesh.GetBlendShapeIndex` 和 `SkinnedMeshRenderer.SetBlendShapeWeight` 动态匹配并恢复原权重；眼睛骨骼沿用同一套名称匹配。没有读写 SMC 私有字段。裙子布料的组件发现、暂停和恢复尚未接入，需先取得实机契约日志再实现。
...
- 2026-09-26：完成阶段 2；抽出共享 VMD 解析器并让相机运行时复用，新增解析器测试（Linux g++ 独立测试通过；Windows 原生模块编译与实机播放待现有构建环境和用户日志验证）。
- 2026-09-26：推进阶段 3 首轮：补齐名称保留和按名称去重，新增跨平台 VMD 采样测试，接入动态层级骨骼播放与姿态恢复；共享测试通过，Windows 原生编译和实机匹配仍待验证。
- 2026-09-26：阶段 4 首轮：为 VMD 播放增加可选 BlendShape 表情匹配与退出恢复；继续保留 SMC/布料私有字段为空，等待实机契约观测。

...
namespace {
    ...
    constexpr uint64_t REPLACEMENT_POSTMODEL_PATH_HASH = 0x0C73F28350A31E2DULL;
    constexpr uint64_t REPLACEMENT_POSTMODEL_BUNDLE_HASH = 0x0B657B21E302A9F9ULL;
    constexpr uint64_t REPLACEMENT_WALK_CLIP_PATH_HASH = 0x0CE4525D14A7046DULL;
    constexpr float REPLACEMENT_SCALE_MULTIPLIER = 1.75f;
    constexpr double REPLACEMENT_WALK_SPEED = 1.5;
    ...
    constexpr const char * REPLACEMENT_POSTMODEL_PATH =
        "assets/beyond/dynamicassets/gameplay/actors/postmodels/characters/"
        "chr_0032_lizhiyan_postmodel.prefab";
    constexpr const char * REPLACEMENT_WALK_CLIP_PATH =
        "assets/beyond/arts/entity/actor/girl/pelica/animations/dialog/"
        "a_actor_pelica_dialog_state_walk.fbx##"
        "a_actor_pelica_dialog_state_walk_loop";
    constexpr const char * TARGET_MODEL_PATH = REPLACEMENT_POSTMODEL_PATH;
    ...
    std::atomic< void * > g_pelicaWalkClip = nullptr;
    uint32_t g_pelicaPrefabGcHandle = 0;
    uint32_t g_pelicaWalkClipGcHandle = 0;
    ...
    static void RunMainResourceDiagnostics( ) {
        ...
        snprintf( checkMessage, sizeof( checkMessage ),
            "[main-diag] Arcane Main CheckExists string=%s hash=%s rawHash=%s",
    ...
    static bool LoadPelicaWalkClip( );
    static bool PrepareCompleteReplacement(
        void * targetRoot, void * replacementRoot, void * clip );
    ...
    static void ReleasePelicaWalkClipRoot( ) {
        if ( g_pelicaWalkClipGcHandle != 0 && api::gchandle_free ) {
            api::gchandle_free( g_pelicaWalkClipGcHandle );
            g_pelicaWalkClipGcHandle = 0;
        }
    }
    ...
    static void TryInstantiateLoadedPelica( ) {
        ...
        void * walkClip = g_pelicaWalkClip.load( );
        ...
        if ( !prefab || !walkClip || !originalInstance || !instantiateMethod ||
        ...
        if ( !ResolveCompleteReplacementApis( ) ) {
            ...
            ReleasePelicaWalkClipRoot( );
            return;
        }

        Log( "[replace-live] login context, Arcane Prefab, and walk clip are ready; instantiating with captured parent" );
        ...
        if ( !instantiateCalled || !replacement ) {
            char message [ 320 ] = { 0 };
            snprintf( message, sizeof( message ),
                "[replace-live] Arcane Instantiate failed called=%s prefab=%p parent=%p method=%p",
        ...
        __try {
            prepared = PrepareCompleteReplacement(
                originalInstance, replacement, walkClip );
        }
        __except ( EXCEPTION_EXECUTE_HANDLER ) {
            prepared = false;
        }
        ...
        if ( !prepared ) {
            ...
            snprintf( failure, sizeof( failure ),
                "[complete-replace] animation setup failed; original actor retained, "
        ...
        __try {
            g_gameObjectSetActive( originalInstance, false, nullptr );
            originalHidden = true;
        }
        __except ( EXCEPTION_EXECUTE_HANDLER ) {
        ...
        if ( !originalHidden ) {
            ...
            Log( "[complete-replace] failed to hide original actor; replacement rolled back" );
            ReleasePelicaPrefabRoot( );
            ReleasePelicaWalkClipRoot( );
            return;
        }

        g_originalLoginActorDisabled = true;
        g_originalRenderersDisabled = false;
        g_targetRoot = originalInstance;
        g_replacementRoot = replacement;
        g_replacementActive = true;

        char message [ 448 ] = { 0 };
        snprintf( message, sizeof( message ),
            "[complete-replace] Arcane own skeleton is playing walk clip "
            "prefab=%p clip=%p parent=%p replacement=%p original=%p; "
            "original root disabled",
            prefab, walkClip, parent, replacement, originalInstance );
        Log( message );
        ReleasePelicaPrefabRoot( );
        ReleasePelicaWalkClipRoot( );
    }
    ...
    static void TryDeferredMainLoad( ) {
        ...
        Log( "[deferred-load] Main hashes ready; loading Arcane for the first time on the captured game thread" );
        ...
        char message [ 288 ] = { 0 };
        snprintf( message, sizeof( message ),
            "[deferred-load] Arcane Main load called=%s handle=%s raw=%016llX:%016llX:%08X",
    ...
    static void * __fastcall CachedLoadGameObjectHook( void * result,
        ...
        if ( captureLoginLoad ) {
            ...
            Log( "[deferred-load] female login actor preserved; Arcane load queued until Main initialization" );
            if ( !scopeSet )
    ...
    static bool LoadPelicaWalkClip( ) {
        if ( g_walkClipResolved.load( ) )
            return g_pelicaWalkClip.load( ) != nullptr;
        ...
        if ( !typeObject || !g_resourceManagerLoadHashType ||
            ...
            Log( "[walk-load] AnimationClip type or resource APIs are unavailable" );
            return false;
        }
        ...
        __try {
            g_resourceManagerLoadHashType( handleBuffer,
                REPLACEMENT_WALK_CLIP_PATH_HASH, typeObject,
        ...
        bool nonZero = readable && ( raw0 != 0 || raw1 != 0 || raw2 != 0 );
        char loadMessage [ 320 ] = { 0 };
        snprintf( loadMessage, sizeof( loadMessage ),
            "[walk-load] AnimationClip load called=%s handle=%s "
            "hash=%016llX raw=%016llX:%016llX:%08X",
            called ? "true" : "false", nonZero ? "non-zero" : "zero",
            static_cast< unsigned long long >( REPLACEMENT_WALK_CLIP_PATH_HASH ),
            static_cast< unsigned long long >( raw0 ),
            static_cast< unsigned long long >( raw1 ), raw2 );
        Log( loadMessage );
        if ( !nonZero )
        ...
        if ( !getCalled || !clip ) {
            Log( "[walk-load] FAssetProxyHandle.Get returned no AnimationClip" );
            return false;
        }

        g_pelicaWalkClip = clip;
        g_walkClipResolved = true;
        if ( api::gchandle_new )
            g_pelicaWalkClipGcHandle = api::gchandle_new( clip, false );
        char clipMessage [ 256 ] = { 0 };
        snprintf( clipMessage, sizeof( clipMessage ),
            "[walk-load] Pelica state-walk AnimationClip=%p gcHandle=%u path=%s",
            clip, g_pelicaWalkClipGcHandle, REPLACEMENT_WALK_CLIP_PATH );
        Log( clipMessage );
        return true;
    }
    ...
    static bool StartRawWalkPlayable( void * replacementRoot, void * clip ) {
        ...
        if ( !animator ) {
            Log( "[walk-playable] no Animator was found or added" );
            return false;
        }
        ...
        void * graphName = api::string_new
            ? api::string_new( "EF Arcane Walk" ) : nullptr;
        void * outputName = api::string_new
            ? api::string_new( "EF Arcane Walk Output" ) : nullptr;
        ...
        __try {
            ...
            if ( outputCreated ) {
                ...
                g_playableOutputSetSource( &output, &playable, 0, nullptr );
                g_playableHandleSetWrapMode( &playable, 1, nullptr );
                g_playableHandleSetSpeed(
                    &playable, REPLACEMENT_WALK_SPEED, nullptr );
        ...
        if ( !configured ) {
            ...
            char failure [ 256 ] = { 0 };
            snprintf( failure, sizeof( failure ),
                "[walk-playable] graph setup failed animatorCount=%d "
                "animatorAdded=%s playable=%s output=%s",
                animatorCount, animatorAdded ? "true" : "false",
        ...
        DestroyWalkPlayable( );
        g_walkPlayableGraph = graph;
        g_walkPlayableStarted = true;
        char success [ 256 ] = { 0 };
        snprintf( success, sizeof( success ),
            "[walk-playable] graph started animator=%p existingAnimators=%d "
            "animatorAdded=%s graph=%p version=%u "
            "updateMode=UnscaledGameTime speed=%.2f",
            animator, animatorCount, animatorAdded ? "true" : "false",
            graph.handle, graph.version, REPLACEMENT_WALK_SPEED );
        Log( success );
        return true;
    }

    static bool StartWalkPlayable( void * replacementRoot, void * clip ) {
        if ( !replacementRoot || !clip )
            return false;

        DestroyWalkPlayable( );
        Log( "[walk-playable] using original Pelica raw PlayableGraph path" );
        return StartRawWalkPlayable( replacementRoot, clip );
    }
    ...
    static bool PrepareCompleteReplacement( void * targetRoot,
        void * replacementRoot, void * clip ) {
        ...
        if ( !SetActive( replacementRoot, true ) )
            return false;
        return StartWalkPlayable( replacementRoot, clip );
    }
    ...
    static void ReplaceNow( ) {
        ...
        Log( "[replace] starting Arcane replacement on Unity main thread" );
        if ( !ResolveApis( ) ) {
        ...
        g_replacementActive = true;
        Log( "[replace] Arcane renderers are active; original animation remains in control" );
    }
...
namespace ModelReplacer {
    ...
    bool Initialize( HMODULE module ) {
        ...
        Log( "[deferred-load] login actor capture armed; Arcane will attach after Main initialization" );
        return true;
    }
    ...
    void QueueReplace( ) {
        ...
        Log( g_mainCategoryHit.load( )
            ? "[deferred-load] status: deferred Arcane Main load was issued"
            : "[deferred-load] status: deferred Arcane Main load was not issued" );
        ...
        snprintf( attachMessage, sizeof( attachMessage ),
            ...
            g_pelicaInstance.load( ), g_pelicaWalkClip.load( ) );
        Log( attachMessage );

---
name: bem-creator
description: Convert supported Endfield source Mods to BEMv1, inspect and validate packages, unpack editable projects, repack them, and assemble multi-Mod ZIP distributions using the BEM creator CLI. Use for BEM creation and conversion diagnostics.
---
...
source-format support and profile requirements. Read [the wire specification](references/BEM_V1_SPEC.md)
when editing a project manifest or diagnosing resource validation failures.
...
- `validate package.bem --report report.json`: validate all appearances and resource bytes.
...
and appearance IDs when updating; use a new package ID for an independent alternative.
...
One package targets one character and contains complete fixed appearances. ZIP may
contain several independently managed packages. Multiple packages for one character
may be installed, but only one enabled. Source key combinations are not automatically
enumerated as appearances.

The runtime supports only BEMv1, currently Windows x64. It retains native shaders and
...
Successful pack/convert validation does not establish live rendering or Android support.
